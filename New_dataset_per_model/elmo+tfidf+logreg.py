...
reg.fit(res_train_x,Train_Y)
dec_function=reg.decision_function(res_test_x)
...
reg=LogisticRegression(class_weight='balanced')
reg.fit(res_train_x,[new_dataset[i][2] for i in range(len(new_dataset))])
pred=reg.predict(res_test_x)
