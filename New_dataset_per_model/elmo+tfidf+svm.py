...
res_test_x=[scraped_posts_emb[i]+list(test_x[i]) for i in range(len(scraped_posts_text))]

interval=0.9
SVM = svm.SVC(C=1.0, kernel='linear', degree=3, gamma='auto',class_weight='balanced')
SVM.fit(res_train_x,Train_Y)
dec_function=SVM.decision_function(res_test_x)
positive_emb=[scraped_posts_emb[i] for i in range(len(res_test_x)) if dec_function[i]>interval]
negative_emb=[scraped_posts_emb[i] for i in range(len(res_test_x)) if dec_function[i]<-interval]
positive_text=[scraped_posts_text[i] for i in range(len(res_test_x)) if dec_function[i]>interval]
negative_text=[scraped_posts_text[i] for i in range(len(res_test_x)) if dec_function[i]<-interval]
...
SVM = svm.SVC(C=1.0, kernel='linear', degree=3, gamma='auto',class_weight='balanced')
SVM.fit(res_train_x,[new_dataset[i][2] for i in range(len(new_dataset))])
pred=SVM.predict(res_test_x)
